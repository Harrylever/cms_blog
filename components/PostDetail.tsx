...
const PostDetail = ({post}: {post: PostNode}) => {
	const getContentFragment = (index: number, text: string, obj: Record<string, undefined>, type?: string) => {
			...
			<div className='px-4 lg:px-0'>
				<div className='flex items-center mb-8 w-full'>
					<div className='flex items-center justify-center mt-2 mb-7 lg:mb-0 w-full lg:w-auto mr-8'>
						<img
							src={post.author ? post.author.photo.url : ''}
							alt={post.author ? post.author.name : ''}
							width='75px'
							style={{maxWidth: '75px', minHeight: '75px', maxHeight: '75px'}}
							className='align-middle rounded-full'
						/>
						<p className='inline align-middle text-gray-700 ml-2 text-lg'>{post.author ? post.author.name : ''}</p>
					</div>
